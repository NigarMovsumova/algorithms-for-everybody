...
public class BinarySearch {
    ...
    private static int iterativeBinarySearch(int [] nums, int target) {
        ...
        logger.log(createLog(format("iterativeBinarySearch start for target : %s in nums: %s", target,
    ...
    private static int recursiveBinarySearch(int [] nums, int target, int left, int right) {
        if (nums == null) return -1;
        logger.log(createLog(format("recursiveBinarySearch start for target : %s in nums: %s", target,
                Arrays.toString(nums))));
    ...
    public static void main(String[] args) {
            ...
            logger.log(createLog(String.valueOf(iterativeBinarySearch(randomNumbers, 5))));
            logger.log(createLog(String.valueOf(iterativeBinarySearch(predefinedNumbers, 5))));

            logger.log(createLog(String.valueOf(recursiveBinarySearch(randomNumbers, 5, 0, 5))));
            logger.log(createLog(String.valueOf(recursiveBinarySearch(predefinedNumbers, 5, 0, 5))));
